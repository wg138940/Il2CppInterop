// modified version of MonoMod.Utils.Cil.CecilILGenerator
// in .NET >= 9.0, LocalBuilder is abstract and has a
// private implementation class (System.Reflection.Emit.RuntimeLocalBuilder)
//
// replaced typeof(LocalBuilder) with fallback to RuntimeLocalBuilder
// https://github.com/MonoMod/MonoMod.Common/blob/ea24867bb49621372eaf53b2ef552cbf488af55b/Utils/Cil/CecilILGenerator.cs#L30
...
sealed class CecilILGenerator : ILGeneratorShim {
    private static Type typeofLocalBuilder
    {
        get
        {
            var typ = typeof(LocalBuilder);
            if (!typ.IsAbstract)
            {
                return typ;
            }

            Assembly coreLib = typ.Assembly;
            Type runtimeLocalBuilderType = coreLib.GetType("System.Reflection.Emit.RuntimeLocalBuilder", throwOnError: false);

            if (runtimeLocalBuilderType == null)
            {
                Logger.Instance.LogError("CecilILGenerator: LocalBuilder is abstract but no implementation class of LocalBuilder found!");
            }

            return runtimeLocalBuilderType;
        }
    }
    private static readonly ConstructorInfo c_LocalBuilder =
        typeofLocalBuilder?.GetConstructors(BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance)
        .OrderByDescending(c => c.GetParameters().Length).First();
    private static readonly FieldInfo f_LocalBuilder_position =
        typeofLocalBuilder?.GetField("position", BindingFlags.NonPublic | BindingFlags.Instance);
    private static readonly FieldInfo f_LocalBuilder_is_pinned =
        typeofLocalBuilder?.GetField("is_pinned", BindingFlags.NonPublic | BindingFlags.Instance);
